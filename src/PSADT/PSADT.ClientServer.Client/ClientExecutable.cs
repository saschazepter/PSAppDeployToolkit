                                        ...
                                        case PipeCommand.PromptToCloseApps:
                                            {
                                                ...
                                                PromptToCloseAppsPayload payload = (PromptToCloseAppsPayload)request.Payload!; List<Exception> failures = [];
                                                foreach (WindowInfo window in WindowUtilities.GetProcessWindowInfo(null, null, null, [.. closeAppsDialogState.RunningProcessService.RunningProcesses.Select(rp => rp.Process.Id).Distinct()]))
                                                {
                                                    ...
                                                    closeAppsDialogState.LogAction($"Stopping process [{runningApp.ProcessName}] with window title [{window.WindowTitle}] and prompt to save if there is work to be saved (timeout in [{payload.Timeout}] seconds)...", LogSeverity.Info);
                                                    try
                                                    {
                                                    ...
                                                    while (true)
                                                    {
                                                        if (promptToCloseStopwatch.Elapsed >= payload.Timeout)
                                                        {
                                                            closeAppsDialogState.LogAction($"Exceeded the [{payload.Timeout.TotalSeconds}] seconds timeout value for the user to save work associated with process [{runningApp.ProcessName}] with window title [{window.WindowTitle}].", LogSeverity.Warning);
                                                            break;
                                                        }
                                                ...
                                                if (failures.Count == 0 && closeAppsDialogState.RunningProcessService.RunningProcesses is IReadOnlyList<RunningProcess> runningApps && runningApps.Count > 0)
                                                {
                                                    ...
                                                    foreach (RunningProcess runningApp in closeAppsDialogState.RunningProcessService.RunningProcesses)
                                                    {
                                                        ...
                                                        if (!runningApp.Process.HasExited)
                                                        {
                                                            ...
                                                            runningApp.Process.WaitForExit();
                                                        }
                                                    }
                                                }
                                                WriteSuccess(true);
