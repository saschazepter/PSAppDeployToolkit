    ...
    public static class SessionManager
    {
        ...
        internal static SessionInfo? GetSessionInfo(in WTS_SESSION_INFOW session)
        {
            ...
            if (isConsoleSession)
            {
                ...
                {
                    idleTime = new(long.Parse(ProcessManager.LaunchAsync(new(typeof(SessionInfo).Assembly.Location.Replace(".dll", ".ClientServer.Client.exe"), new(["/GetLastInputTime"]), null, new(ntAccount, sid, session.SessionId), false, false, false, false, false, false, false, null, true))!.Task.GetAwaiter().GetResult().StdOut!.First()));
                }
            }
