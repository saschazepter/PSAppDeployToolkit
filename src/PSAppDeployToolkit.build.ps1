...
Add-BuildTask Build {
    ...
    if (($canSign = ($env:GITHUB_ACTIONS -eq 'true') -and ($env:GITHUB_REF_NAME -match '^(main|develop|4.0.x)$')))
    {
        ...
        Get-ChildItem -Path $Script:BuildModuleRoot -Include '*.ps*1', 'PSADT*.dll', 'PSADT.ClientServer.Client.exe', 'iNKORE.UI.WPF*.dll', 'Deploy-Application.exe', 'Invoke-AppDeployToolkit.exe' -Recurse | ForEach-Object {
